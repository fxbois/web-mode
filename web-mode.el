...
(defun web-mode-snippet-codes ()
  ...
  (let (codes
    ...
    (dotimes (counter l)
      ...
      (add-to-list 'codes (list (nth 0 snippet) (1+ counter))))
    ;;    (message "%S" codes)
    codes))
...
(defun web-mode-match-velocity-tag ()
  "Match velocity tag."
  (let ((regexp (concat "#" (regexp-opt web-mode-velocity-controls))))
    (if (looking-at-p "#end")
...
(defun web-mode-match-ctemplate-tag ()
  "Match ctemplate tag."
  (looking-at "{{[#^/]\\([[:alnum:]_]+\\)")
  (let ((regexp (concat "{{[#^/]" (match-string-no-properties 1))))
...
(defun web-mode-on-after-change (beg end len)
  ...
  (let ((chunk "")
    ...
    (if (not (= (point-max) (1+ (buffer-size))))
	(setq web-mode-is-narrowed t)
	(when (= (point-max) (1+ (buffer-size)))
	  ...
	  (when (and (not web-mode-disable-autocompletion)
		...
		;;            ) ;; when
	      ...
	      (when jump-pos (goto-char jump-pos)
...
(defun web-mode-rsb-client (regexp &optional limit noerror)
  ...
  (catch 'return-val
    (while t
      (let ((ret (re-search-backward regexp limit noerror)))
	(when (or (null ret)
		  (not (get-text-property (point) 'server-side)))
	  (throw 'return-val ret))))))
...
(defun web-mode-rsf-client (regexp &optional limit noerror)
  ...
  (catch 'return-val
    (while t
      (let ((ret (re-search-forward regexp limit noerror)))
	(when (or (null ret)
...
(defun web-mode-sf-client (expr &optional limit noerror)
  ...
  (catch 'return-val
    (while t
      (let ((ret (search-forward expr limit noerror)))
	(when (or (null ret)
...
(defun web-mode-sb (expr &optional limit noerror)
  ...
  (catch 'return-val
    (while t
      (let ((ret (search-backward expr limit noerror)))
	(when (or (null ret)
		  (not (web-mode-is-comment-or-string)))
	  (throw 'return-val ret))))))
