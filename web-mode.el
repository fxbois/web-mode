  ...
  (let ((keymap (make-sparse-keymap)))
    ...
    (define-key keymap (kbd "C-$") 
      (lambda ()
        (interactive)
        (message 
         "cs=%S cl=%S ct=%S ** ss=%S sl=%S st=%S ** mt=%S ** %S ** is-c-o-s=%S" 
   ...
   (save-excursion
     (save-match-data
       (let ((inhibit-modification-hooks t)
         ...
         (remove-text-properties beg end '(markup-type nil client-language nil server-language nil client-side nil server-side nil client-type nil server-type nil face nil))
...
;;todo: scanner directement dans cette boucle pour éviter <?php $s = '?>'; ?> : "//\\|/\\*\\|\"\\|'\\|\\?>"
...
;; (defun web-mode-scan-server-old (beg end)
;;   "Scan server side blocks (PHP / JSP / ASP) and identifies strings and comments."
;;   (save-excursion

;;     (let (open close ms regexp props closing-string start sub2 pos tagopen continue)
...
;;           (setq closing-string "?>"
;;                 regexp "//\\|/\\*\\|\"\\|'\\|<<<['\"]?\\([[:alnum:]]+\\)['\"]?"
;;                 props '(server-language php server-side t font-lock-face nil)))

;;          ((string= "<%-" tagopen) 
;;           (setq closing-string "--%>" 
;;                 props '(server-side t server-type comment font-lock-face web-mode-comment-face)))
...
;;           (setq closing-string "%>" 
;;                 regexp "//\\|/\\*\\|\"\\|'"
;;                 props '(server-language directive server-side t font-lock-face nil)))
         
;;          ((string= "<%" sub2) 
;;           (setq closing-string "%>"
;;                 regexp "//\\|/\\*\\|\"\\|'"
;;                 props (if (string= web-mode-server-language "asp")
;;                           '(server-language asp server-side t font-lock-face nil)
;;                         '(server-language jsp server-side t font-lock-face nil))))
         
;;          ((member sub2 '("${" "#{"))
;;           (setq closing-string "}"
;;                 props '(server-language jsp server-side t font-lock-face nil))
;;           )
         
;;          ((string= "{{" sub2)
;;           (setq closing-string "}}"
;;                 props '(server-side t font-lock-face nil))
;;           )
         
;;          ((string= "{%" sub2)
;;           (setq closing-string "%}"
;;                 regexp "//\\|/\\*\\|\"\\|'"
;;                 props '(server-language engine server-side t font-lock-face nil))
;;           )
...
;;                ((string= fc "'")
;;                 (setq props '(server-type string font-lock-face web-mode-string-face))
;;                 (while (and continue (search-forward "'" close t))
;;                   (setq continue (char-equal ?\\ (char-before (- (point) 1))))
;;                   )
;;                 )
...
;;                ((string= ms "//")
;;                 (setq props '(server-type comment font-lock-face web-mode-comment-face))
;;                 (goto-char (if (< close (line-end-position)) close (line-end-position)))
;;                 )
               
;;                ((string= ms "/*")
;;                 (setq props '(server-type comment font-lock-face web-mode-comment-face))
;;                 (search-forward "*/" close t)
;;                 )
               
;;                ((string= fc "<")
;; ;;                (message "tag=%S" (match-string 1))
;;                 (setq props '(server-type string font-lock-face web-mode-string-face))
;;                 (re-search-forward (concat "^" (match-string 1)) close t)
;;                 )
...
;; ;; todo: indentify node-type: opening-tag closing-tag attr-name attr-value
;; (defun web-mode-scan-html (beg end)
;;   "Scan html nodes (attributes)."
;;   (save-excursion
;;     (let (ab ae continue)
;;       (goto-char beg)
;;       (while (re-search-forward "\\([[:alpha:]][[:alnum:]_-]+=\"[^\"]*\"\\)" end t)
;;         (if (or (get-text-property (match-beginning 0) 'client-side)
;;                 (get-text-property (match-beginning 0) 'server-side))
;;             (setq continue t)
;;           (setq ab (match-beginning 0)
;;                 ae (match-end 0))
;;           (when (or (get-text-property ae 'client-side)
;;                     (get-text-property ae 'server-side))
;;             (setq ae nil 
;;                   continue t)
;;             (while (and continue (search-forward "\"" end t))
;;               (if (and (not (get-text-property (point) 'client-side))
;;                        (not (get-text-property (point) 'server-side)))
;;                   (setq ae (point)
;;                         continue nil)
;;                 ))
;;             );;when
;;           (if ae
;;               (put-text-property ab ae 'client-type 'attr))
;;           );;if
;;         );;while
;;       )))

;; (defun web-mode-scan-client2 (beg end)
;;   "Scan client side blocks (JS / CSS / HTML Comments) and identifies strings and comments."
;;   (save-excursion
;;     (let (open close continue sub3 ms regexp props closing-string start)

;;       (goto-char beg)
      
;;       (while (re-search-forward "<\\(!--\\|!D\\|style[^>]*>\\|script[^>]*>\\)" end t)
...
;;               regexp nil
;;               sub2 (substring (match-string 0) 0 2)
;;               sub3 (substring (match-string 0) 0 3)
;;               )
...
;;                 props '(client-side t client-type comment font-lock-face web-mode-comment-face)))

;;          ((string= "<!D" sub3)
;;           (setq closing-string ">" 
;;                 open (match-beginning 0)
;;                 props '(client-side t client-type doctype font-lock-face web-mode-doctype-face)))

;;          );;cond
        
;;         (when (and closing-string (search-forward closing-string end t))
;;           (setq close (if (member sub2 '("<!")) (match-end 0) (match-beginning 0))
;;                 pos (point))
;;           (add-text-properties open close props)        
;;           );; when
                
;;         (when (and regexp close)
;;           (goto-char open)
...
;;              ((string= fc "'")
;;               (setq props '(client-type string font-lock-face web-mode-string-face))
;;               (while (and continue (search-forward "'" close t))
;;                 (setq continue (char-equal ?\\ (char-before (- (point) 1))))
;;                 )
;;               )
...
;;              ((string= ms "//")
;;               (setq props '(client-type comment font-lock-face web-mode-comment-face))
;;               (goto-char (if (< close (line-end-position)) close (line-end-position)))
;;               )
...
;;     (message "web-mode-highlight-client-blocks")
;;     (setq limit (point-max))
...
;; ;;          (message "%s" chunk)
;;           (goto-char open)
;;           (while (and (re-search-forward "{\\([^}]+\\)}" close t)
;;                       (< (point) pos))
...
;;           (goto-char open)
;;           (while (and (not web-mode-disable-css-colorization)
;;                       (re-search-forward "#[0-9a-fA-F]\\{6\\}\\|#[0-9a-fA-F]\\{3\\}\\|rgb([ ]*\\([[:digit:]]\\{1,3\\}\\)[ ]*,[ ]*\\([[:digit:]]\\{1,3\\}\\)[ ]*,[ ]*\\([[:digit:]]\\{1,3\\}\\)\\(.*?\\))" close t)
;;                       (< (point) pos))
;;             (web-mode-colorize (match-beginning 0) (match-end 0))
;;             );;while
;;           (goto-char pos)
...
;;        ((string= "{%" ms)
;;         (setq font-lock-keywords web-mode-engine-font-lock-keywords
;;               closing-string "%}"
;;               type 'php))
...
;;             (font-lock-default-unfontify-region open-out close-out)
;; ;;            (font-lock-default-unfontify-region open close)
;;             (font-lock-fontify-region open-out close-out)
;;             close-out
;;             )
...
;;todo: try avoid use t t
;;todo: "\\([$#{]{\\)\\([^}]+\\)\\(}}?\\)" place in scan-server

;;(defconst web-mode-html-font-lock-keywords
;;  (list
;;   '("</?[[:alpha:]][[:alnum:]:_]*?\\(/?>\\|<\\|[ ]\\|/\\|$\\)" 0 'web-mode-html-tag-face)
;;   '("^<\\(!D\\|\\?x\\)[^>]*>" 0 'web-mode-doctype-face)
;;   '("[[:alnum:]\">} ]\\(/?>\\)" 1 'web-mode-html-tag-face)
;;   '("\\(</?[^%]\\|/?>\\)" 0 'web-mode-html-tag-face t t)
;;   '("[[:space:]>]\\([[:alnum:]_-]+=\\)\\(\"[^\"]*\"\\)?" (1 'web-mode-html-attr-name-face) (2 'web-mode-html-attr-value-face t t))
;;   '("\\([$#{]{\\)\\([^}]+\\)\\(}}?\\)" (1 'web-mode-preprocessor-face t t) (2 'web-mode-variable-name-face t t) (3 'web-mode-preprocessor-face t t))
...
;;   '("<%--\\(.\\|\n\\)*?%>" 0 'web-mode-comment-face t t)
;;   '("{#\\(.\\|\n\\)*?#}" 0 'web-mode-comment-face t t)
;;   '("</?\\([#@][[:alpha:]._]+\\|[[:alpha:]]+[:_][[:alpha:]]+\\)" 1 'web-mode-preprocessor-face t t)
;;   ))
