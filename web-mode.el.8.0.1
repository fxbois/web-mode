...
(defconst web-mode-version "8.0.2"
  "Web Mode version.")
...
(defvar web-mode-text-properties
  '(part-side nil part-token nil part-language nil tag-name nil tag-type nil tag-beg nil tag-end nil block-side nil block-token nil block-beg nil block-end nil face nil font-lock-face nil)
  "Text properties used for fontification and indentation.")

(defvar web-mode-text-properties2
  '(part-side nil part-token nil part-language nil tag-name nil tag-type nil tag-beg nil tag-end nil face nil font-lock-face nil)
  "Text properties used for fontification and indentation.")
      ...
      (while (web-mode-rsf-client regexp end t)
        ...
        (cond
         ...
         ((string= tname "?xml")
          (setq regexp regexp2
                expr "?>"
                props '(tag-name "doctype" tag-type void font-lock-face web-mode-doctype-face)))
...
;;          (setq expr ">")
         ...
         (t
          ...
          (cond
           ((eq ?\/ (string-to-char tname))
            (setq props (list 'font-lock-face tface 'tag-name (substring tname 1) 'tag-type 'end)
                  slash-beg t)
            (setq limit (if (> end (line-end-position)) (line-end-position) end))
            )
           ((web-mode-is-void-element tname)
            (setq props (list 'font-lock-face tface 'tag-name tname 'tag-type 'void))
            )
           (t
            (setq props (list 'font-lock-face tface 'tag-name tname 'tag-type 'start))
            )
           );cond
          );t
         );cond
        ...
        (if is-tag
            (progn
              (add-text-properties
               tbeg (1+ tbeg)
               '(tag-beg t font-lock-face web-mode-html-tag-bracket-face))
              (when slash-beg
                (add-text-properties
                 (+ tbeg 1) (+ tbeg 2)
              ...
              (when close-found
                (add-text-properties
                 (1- tend) tend
                 '(tag-end t font-lock-face web-mode-html-tag-bracket-face))
                );when
              (when slash-end
                (add-text-properties
                 (- tend 2) (- tend 1)
          ...
          (cond
           ...
           ((eq ?\' ch-at)
            (unless (eq ?\\ ch-before)
              ...
              (cond
               ((string= content-type "javascript")
                (setq props '(part-token string font-lock-face web-mode-javascript-string-face)))
               ((string= content-type "css")
                (setq props '(part-token string font-lock-face web-mode-css-string-face)))
               ((string= content-type "json")
                (setq props '(part-token string font-lock-face web-mode-json-string-face)))
               (t
                (setq props '(part-token string font-lock-face web-mode-part-string-face)))
               );cond
              );unless
            )
                ...
                (if (looking-at-p "[ ]*:")
                    (cond
                     ((eq ?\@ (char-after (1+ start)))
                      (setq props '(part-token string font-lock-face web-mode-json-context-face))
                      )
                     (t
                      (setq props '(part-token string font-lock-face web-mode-json-key-face))
                      )
                     )
                  (setq props '(part-token string font-lock-face web-mode-json-string-face)))
                )
           ...
           ((eq ?\/ ch-next)
            (unless (eq ?\\ ch-before)
              (setq props '(part-token comment font-lock-face web-mode-part-comment-face)
                    token-type "comment")
   ...
   (t
    ...
    (if (or (= state 3)
        ...
        (add-text-properties name-beg name-end
                             (list 'part-token 'attr 'font-lock-face
      ...
      (add-text-properties name-beg name-end
                           '(part-token attr font-lock-face web-mode-html-attr-name-face)))
    (when (and val-beg val-end)
      (setq val-end (if (eq ?\> char) val-end (1+ val-end)))
      (add-text-properties val-beg val-end '(part-token attr font-lock-face web-mode-html-attr-value-face))
      (put-text-property name-end val-beg 'font-lock-face 'web-mode-html-attr-equal-face))
    );t
   );cond
  )
  ...
  (let (str plist len)
    ...
    (cond
     ((string= (substring str 0 1) "#")
      ...
      (put-text-property beg end 'font-lock-face plist))
  ...
  (let (beg end tag pos l n tags i cont cell overlay overlays first
    ...
    (when overlays
      (dolist (overlay overlays)
        (when (eq (overlay-get overlay 'font-lock-face) 'web-mode-warning-face)
